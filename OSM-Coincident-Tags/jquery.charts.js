(function($)
{
	...
	$.fn.charts = function (settings) {
		...
		function GetVerticalBarsOutput(groupArray, labelArray, valueArray, totalValue, largestValue, labelTextArray) {
			...
			if (config.showgrid) {
				var gridLineHeight = Math.floor(config.chartheight / config.gridlines);
				for (var i = 0; i < config.gridlines; i++) {
					var alternatingClass = "odd";
					if (i%2 == 0) {
						alternatingClass = "even";
					}
					output += "<div class=\"" + config.classmodifier + "gridline " + alternatingClass + "\" style=\"height: " + gridLineHeight + "px; background-color: Black;\"></div>";
				}
			}
